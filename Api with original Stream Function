...
// register API
function register($user, $pass, $token) {
...
function login($user, $pass) {
	...
	//This is how you get data from a table etc
...
//upload API
function upload($id, $photoData, $title, $token) {
	...
	if ($photoData['error']==0) {
		...
		$result = query("INSERT INTO photos(IdUser,title,device_token) VALUES('%d','%s','%s')", $id, $title, $token);
		if (!$result['error']) {
...
function stream($IdPhoto=0) {
	...
	if ($IdPhoto==0) {
		...
		//This php statement should return to its original form while also supplementing the php statement below
